...
def make_back_cover():
    with BuildPart() as bc:
        ...
        #     MAG_OUTER_WALL+MAG_T]; 从内侧 (+z) 嵌磁后被 plug/PCB 压住. 不穿透外表面 (保薄壁吸力).
        #     盖板仅 1.5mm 装不下 3mm 深腔 => 先在内侧 (+z) 长磁柱 boss 补料 (Φ磁+2壁), 再掏腔.
        MAG_BOSS_D = MAG_D + 2 * 1.6                 # 磁柱外径 (磁 Φ8 + 单边 1.6 壁)
        MAG_BOSS_TOP_Z = MAG_OUTER_WALL + MAG_T + 0.5  # 磁柱顶 z (盖住腔底 + 0.5 余料)
...
def make_bracket():
    with BuildPart() as br:
        ...
        # ============================================================
        # 修复: Type-C 专用实体壳块 (boss) — 不再把母座切进 5mm 薄板
        # ============================================================
        #   bug 复现: MATE_T=5.0 (板厚 Z) < TYPEC_FLANGE_H=10.3 (法兰 Z) => 旧方案切穿薄板.
        #   修复方案: 在对接板底部长一个专用实体壳块完整包裹母座, 与支架熔为一体 (单一实体):
        #     - 壳块 X 宽 = TYPEC_HOUSING_W (≈19.9, = 法兰宽 + 2 壁); 居中于 POGO/走线腔 (tc_cx=pogo_cx).
        #     - 壳块 Z 厚 = TYPEC_HOUSING_Z (≈13.5, ≥ 法兰高 10.3 + 2 壁); +Z 面齐板顶 (藏屏后).
        #     - 壳块 Y 深 = TYPEC_HOUSING_Y (≈16.8, = 体深 14 + 法兰沉 1.2 + 底壁 1.6).
        #     - 朝向: -Z (背离屏体, 朝支架背后) + -Y (向下) 凸出; +Z 面 < 屏体后表面 (不侵入屏体).
        tc_cx = pogo_cx                                    # 壳块 X 居中 = POGO/走线腔 (供电链对齐)
        # 壳块 Z: +Z 面齐板顶 mate_z_top (= -10.3 < 屏体后表面 back_outer_z = -9.8, 藏屏后),
        #   向 -Z 凸出 TYPEC_HOUSING_Z (凸到支架背后).
        housing_z_top = mate_z_top                         # 壳块 +Z 面 (藏屏后, 不越屏体后表面)
        housing_z_bot = housing_z_top - TYPEC_HOUSING_Z    # 壳块 -Z 面 (凸到背后)
        housing_cz = (housing_z_top + housing_z_bot) / 2
        # 壳块 Y: -Y 面 (开口/法兰面) 齐对接板底边 plate_bot_y, 向 +Y 深入 TYPEC_HOUSING_Y.
        housing_y_min = plate_bot_y                        # 壳块 -Y 面 (底边外缘, 开口朝下)
        housing_y_max = housing_y_min + TYPEC_HOUSING_Y    # 壳块 +Y 面 (体腔顶 + 内壁)
        bottom_y = housing_y_min                           # 底边壁外缘 (Y 最小) = 开孔/法兰所在面
        tc_cz = housing_cz                                 # 口沿 Z 居中于壳块厚度
        # (0) 壳块实体: 整块 Box 并入 bracket (熔为一体, 保持单一连通实体).
        ...
        # ---- 内部走线腔 (从 POGO 凹腔向 -Y 连到 Type-C 体腔顶; 容飞线 + 小 dock PCB/CC 电阻) ----
        ...
        channel_y_bot = body_cav_top_y - 1.0               # 下接 Type-C 体腔顶 (重叠 1mm 保证连通)
...
def main():
    ...
    with BuildPart() as _ch3c:
        ...
        with Locations((_mate['pogo_cx'], _ccy, _mate['z_top'] - CHANNEL_DEPTH / 2)):
    ...
    print(f"[4 TypeC真穿透] *** 最重要 *** (支架底边防水 Type-C — 专用壳块 boss 包裹母座)")
    ...
    # 不撞 POGO: 壳块 Y 区间 vs POGO 凹腔 Y 区间分离.
    pogo_y0_4 = _mate['pogo_cy'] - _mate['pocket_h'] / 2
    pogo_sep = pogo_y0_4 - _mate['housing_y_max']
    print(f"   [4-pogo 分离] 壳块 Y[{_mate['housing_y_min']:.1f},{_mate['housing_y_max']:.1f}] "
          f"vs POGO 凹腔下沿 Y={pogo_y0_4:.1f}; 间隙={pogo_sep:.2f}mm (>0? {pogo_sep > 0})")
