...
// 局刷波形 LUT(写入 0x32, 105B)。来源 GxEPD2_370_TC1(同芯片 SSD1677)。timing 段
// ([50-59]) 为借用值, 真机重影/对比不足时微调。
...
// 局刷: 自定义局刷 LUT(不闪) + 0xCF(mode2 用我们的 LUT, 不重载 OTP)。0x26 同步由
// disp 层做。0x21 差分会让本屏局刷不动, 故不设。残影/灰靠每 N 次全刷洗净。
bool ssd1677_update_partial(void)
{
    hal_spi_cmd(0x3C); hal_spi_data(0xC0);                      // 边框 HiZ
    hal_spi_cmd(0x32);                                          // 自定义局刷 LUT
