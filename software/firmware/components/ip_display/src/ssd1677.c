...
// 全刷: 0x21 bypass 旧 RAM(0x40) + 0x22=0xF7(含 OTP 全刷波形)。
// 按本屏型号官方参考 GxEPD2_426_GDEQ0426T82(同型号 4.26" 800x480 SSD1677)。
bool ssd1677_update_full(void)
{
    hal_spi_cmd(0x21); hal_spi_data(0x40); hal_spi_data(0x00);  // Display Update Ctrl1: bypass 旧RAM
...
// 局刷: 本屏 OTP 自带局刷波形, 不需自定义 LUT。关键是 0x21=0x00(用旧 RAM 0x26
// 与新 RAM 0x24 差分) + 0x22=0xFC, 而非全刷的 0x40/0xF7。0x26 同步由 disp 层做。
// 按本屏型号官方参考 GxEPD2_426_GDEQ0426T82。
bool ssd1677_update_partial(void)
{
    hal_spi_cmd(0x21); hal_spi_data(0x00); hal_spi_data(0x00);  // Display Update Ctrl1: 旧RAM 差分
    hal_spi_cmd(0x22); hal_spi_data(0xFC);
    hal_spi_cmd(0x20);
    vTaskDelay(pdMS_TO_TICKS(10));
    return wait_busy();
}
